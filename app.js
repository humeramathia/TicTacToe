...
var game = {
  ...
  shouldGameContinue: function () {
    ...
    if (
      ...
      game.tiles.every((tile) => tile.isTaken === true) &&
      this.winner === true
    ) {
      view.showWinner(icon);
...
var handlers = {
  ...
  setOpponent: function (event) {
    game.playerEven = new Player(event.target.value, game.playerEven.icon);
    if (game.playerOdd.icon === "X") {
